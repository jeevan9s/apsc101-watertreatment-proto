...
import { getCurrentPhase, normalizePhase } from "@/lib/frontendHandlers";
...
const PHASE_STYLES: Record<string, { color: string; pulse: boolean }> = {
  ...
  treating: { color: "text-cyan-700 bg-cyan-700", pulse: true },
  offline: { color: "text-neutral-400 bg-neutral-400", pulse: false },
  treated: { color: "text-green-400 bg-green-400", pulse: false },
};

export function SystemStatusCard({ payload }: { payload?: any }) {
  const status = (payload?.system?.status || "offline").toLowerCase();
  ...
  return (
    <Card className="bg-neutral-900 border border-neutral-800 rounded-2xl flex-1 transform transition-transform duration-200 ease-in-out hover:scale-105">
      ...
      <CardContent className="flex items-center justify-between py-2">
        <span
          className={`text-sm font-medium ${statusStyle.color.split(" ")[0]}`}
        >
          {status}
        </span>
        <div
...
export function SystemPhaseCard({ payload }: { payload?: any }) {
  ...
  return (
    <Card className="bg-neutral-900 border border-neutral-800 rounded-2xl flex-1 transform transition-transform duration-200 ease-in-out hover:scale-105 ml-2">
      ...
      <CardContent className="flex items-center justify-between py-2">
        <span
          className={`text-sm font-medium ${phaseStyle.color.split(" ")[0]}`}
        >
          {phase}
        </span>
        <div
...
export function SystemStatusAndPhase({ payload }: { payload?: any }) {
  return (
